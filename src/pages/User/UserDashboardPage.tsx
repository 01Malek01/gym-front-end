        ...
        <div className="bg-zinc-900 border border-yellow-700 rounded-xl shadow-2xl p-6">
          ...
          {user?.membershipStatus === "pending" ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-app_primary-dark to-app_primary-electricBlue/80 text-white rounded-lg p-6 shadow-xl border border-app_primary-electricBlue">
                <p className="text-2xl font-extrabold mb-3 uppercase tracking-wide text-app_primary-brightTeal">Current Plan: <span className="text-white">{user?.membershipType?.type}</span></p>
                <p className="text-lg mb-2"><span className="font-semibold text-app_primary-brightTeal/90">Expiration:</span> {user?.membershipExpirationDate}</p>
                <p className="text-lg mb-2"><span className="font-semibold text-app_primary-brightTeal/90">Duration:</span> {user?.membershipType?.durationInDays} days</p>
                {/* You could add a progress bar here if you have days remaining data */}
                {/* <div className="mt-4 h-3 bg-indigo-700 rounded-full">
                  <div className="h-full bg-teal-400 rounded-full" style={{ width: `${(daysRemaining / totalDuration) * 100}%` }}></div>
                </div> */}
              </div>
              {/* Could add another box here for "Next Tier" or "Manage Membership" */}
            </div>
          )}
        </div>
      </div>
